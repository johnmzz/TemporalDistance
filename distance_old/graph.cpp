#include "graph.h"

Graph::Graph(string input_file, string _directed) {
    n = 0;
    m = 0;
    d_min = INT_MAX;
    d_max = 0;
    t_min = INT_MAX;
    t_max = 0;
    
    // directed or undirected
    if (_directed == "U") 
        directed = false;
    else if (_directed == "D")
        directed = true;
    else {
        cout << "Incorrect input!" << endl;
        exit(0);
    }

    // get number of vertices and edges
    ifstream fin("./data/" + input_file + ".txt");
    if (!fin) {
        cout << "Cannot open " << input_file << ".txt" << endl;
        exit(0);
    }
    int u, v, t, d;
    while (fin >> u >> v >> t >> d) {
        n = max(u,n);
        n = max(v,n);
        t_min = min(t, t_min);
        t_max = max(t, t_max);
        d_min = min(d, d_min);
        d_max = max(d, d_max);
        m++;
    }
    fin.close();

    // initialize adj_list, degree
    graph = vector<vector<Edge> >(n+1, vector<Edge>());
    out_degree = vector<int>(n+1);
    if (directed) {
        r_graph = vector<vector<Edge> >(n+1, vector<Edge>());
        in_degree = vector<int>(n+1);
    }

    // read graph
    fin.open("./data/" + input_file + ".txt");
    while (fin >> u >> v >> t >> d) {
        graph[u].push_back(Edge(v, t, d));
        out_degree[u]++;

        if (directed) {
            r_graph[v].push_back(Edge(u, t, d));
            in_degree[v]++;
        }
        else {
            graph[v].push_back(Edge(u, t, d));
            out_degree[v]++;
        }
    }
    if (!directed) in_degree = out_degree;
    
    sort_by_degree(graph);
    sort_by_degree(r_graph);

    //print_graph();
    //print_r_graph();

    // initialize index
    in_label = vector<vector<Label> >(n+1, vector<Label>());
    out_label = vector<vector<Label> >(n+1, vector<Label>());

    cout << "Graph contructed, n = " << n << ", m = " << m << ", d_max = " << d_max << ", t_max = " << t_max << endl;
}

void Graph::sort_by_degree(vector<vector<Edge> > &g) {
    vector<pair<int, int>> v_degree_ordered(g.size());
    for (uint32_t i = 1; i < g.size(); i++) {
        v_degree_ordered[i] = make_pair(i, (in_degree[i] + 1) * (out_degree[i] + 1));
    }
    std::sort(v_degree_ordered.begin(), v_degree_ordered.end(), [](auto &left, auto &right) {return left.second > right.second;});

    vector<int> offset(g.size());
    for (int i = 0; i < g.size(); i++) {
        offset[v_degree_ordered[i].first] = i+1;
    }

    vector<vector<Edge> > new_graph = vector<vector<Edge> > (n+1, vector<Edge>());
    for (int i = 1; i <= n; i++) {
        int u = v_degree_ordered[i-1].first;
        for (int j = 0; j < g[u].size(); j++) {
            int v = offset[g[u][j].v];
            int d = g[u][j].d;
            int t = g[u][j].t;
            new_graph[i].push_back(Edge(v,t,d));
        }
    }
    g = new_graph;
}

void Graph::print_graph() {
    for (int i = 0; i < graph.size(); i++) {
        cout << i << ":[";
        for (auto j : graph[i]) {
            cout << "(" << j.v << "," << j.d << "," << j.t << "),";
        }
        cout << endl;
    }
}

void Graph::print_r_graph() {
    for (int i = 0; i < r_graph.size(); i++) {
        cout << i << ":[";
        for (auto j : r_graph[i]) {
            cout << "(" << j.v << "," << j.d << "," << j.t << "),";
        }
        cout << endl;
    }
}

void Graph::print_labels() {
    cout << "out_label = " << endl;
    for (int i = 0; i < out_label.size(); i++){
        cout << i << ": [";
        for (int j = 0; j < out_label[i].size(); j++) {
            Label l = out_label[i][j];
            cout << "(" << l.v << "," << l.d << "," << l.t1 << "," << l.t2 << "), ";
        }
        cout << "]," << endl;
    }

    cout << "in_label = " << endl;
    for (int i = 0; i < in_label.size(); i++){
        cout << i << ": [";
        for (int j = 0; j < in_label[i].size(); j++) {
            Label l = in_label[i][j];
            cout << "(" << l.v << "," << l.d << "," << l.t1 << "," << l.t2 << "), ";
        }
        cout << "]," << endl;
    }
}

int Graph::span_distance(int u, int v, int t1, int t2) {
    if (u == v) return 0;
    int min_distance = INT_MAX;

    auto it = lower_bound(out_label[u].begin(), out_label[u].end(), Label(v, t1, t2, 0));
    while (it != out_label[u].end() && it -> v == v) {
        if (t1 <= it -> t1 && it -> t2 <= t2 && it -> d < min_distance) {
            min_distance = it -> d;
            break;
        }
        it++;
    }

    it = lower_bound(in_label[v].begin(), in_label[v].end(), Label(u, t1, t2, 0));
    while (it != in_label[v].end() && it -> v == u) {
        if (t1 <= it -> t1 && it -> t2 <= t2 && it -> d < min_distance) {
            min_distance = it -> d;
            break;
        }
        it++;
    }

    for (vector<Label>::iterator it1 = out_label[u].begin(), it2 = in_label[v].begin(); it1 != out_label[u].end() && it2 != in_label[v].end();) {
        if (it1 -> v < it2 -> v) it1++;
        else if (it1 -> v > it2 -> v) it2++;
        else if (!(t1 <= it1 -> t1 && it1 -> t2 <= t2)) it1++;
        else if (!(t1 <= it2 -> t1 && it2 -> t2 <= t2)) it2++;
        else {
            if (it1 -> d + it2 -> d < min_distance) {
                min_distance = it1 -> d + it2 -> d;
            }
            it1++;
            it2++;
        }
    }
    return min_distance;
}

void Graph::construct() {
    clock_t start, end;
    start = clock();

    for (int i = 1; i <= n; i++) {
        cout << "construct for vertex i = " << i << endl;
        Q.push(Quad(i,0,-1,-1));
        construct_for_a_vertex(graph, in_label, i, false);

        if (directed) {
            Q.push(Quad(i,0,-1,-1));
            construct_for_a_vertex(r_graph, out_label, i, true);
        }
        //cout << ", finished!" << endl;
    }

    end = clock();
    cout << "build index: " << (float)(end - start) / CLOCKS_PER_SEC << " s" << endl;
}
void Graph::construct_for_a_vertex(vector<vector<Edge> > &g, vector<vector<Label> > &label, int u, bool reverse) {
    //int i = 0;
    while (!Q.empty()) {
        Quad quad = Q.top();
        //cout << i << " pop: (v=" << quad.v << ", d=" << quad.d << ", t1=" << quad.t1 << ", t2=" << quad.t2 << ")" << endl;
        Q.pop();
        int v = quad.v;
        if (u != v && quad.t2 - quad.t1 > 0) {
            if ((!reverse && span_distance(u, v, quad.t1, quad.t2) <= quad.d) || (reverse && span_distance(v, u, quad.t1, quad.t2) <= quad.d)) {
                continue;
            }
            else {
                add_label(label, v, u, quad.d, quad.t1, quad.t2);
                //cout << i << " add label: L(" << v << ") += (u=" << u << ", d=" << quad.d << ", t1=" << quad.t1 << ", t2=" << quad.t2 << ")" << endl;
            }
        }
        for (Edge e : g[v]) { 
            if (u < e.v) {
                int _t1 = (quad.t1 == -1? e.t : min(quad.t1, e.t));
                int _t2 = (quad.t2 == -1? e.t : max(quad.t2, e.t));
                int _d = quad.d + e.d;

                // if use max_d_from_u[v], then need to specify (v, t1, t2), maybe use nested hash
                Q.push(Quad(e.v, _d, _t1, _t2));
                //cout << i << " push: (v=" << e.v << ", d=" << _d << ", t1=" << _t1 << ", t2=" << _t2 << ")" << endl;
            }
        }
        // i++;
        // cout << endl;
    }
}

void Graph::add_label(vector<vector<Label> > &label, int u, int v, int d, int t1, int t2) {
    if (!label[u].size()) {
        label[u].push_back(Label(v, d, t1, t2));
    }
    else {
        Label temp (v, d, t1, t2);
        auto it = upper_bound(label[u].begin(), label[u].end(), temp);
        label[u].insert(it, temp);
    }
}

int Graph::min_distance(vector<int>& dist, vector<bool>& visited) {
    int min_dist = INT_MAX;
    int idx = INT_MAX;
    for (int i = 0; i < dist.size(); i++) {
        if (!visited[i] && dist[i] < min_dist) {
            min_dist = dist[i];
            idx = i;
        }
    }
    return idx;
}

int Graph::temporal_dijkstra(int s, int t, int t1, int t2) {
    vector<int> dist = vector<int>(graph.size(), INT_MAX);
    vector<bool> visited = vector<bool>(graph.size(), false);

    dist[s] = 0;

    while (!visited[t]) {
        int v = min_distance(dist, visited);
        //cout << "this round v = " << v << endl;
        
        if (v == INT_MAX) break;

        visited[v] = true;
        for (auto e : graph[v]) {
            if (!visited[e.v] && dist[v]+e.d < dist[e.v] && e.t >= t1 && e.t <= t2) {
                dist[e.v] = dist[v] + e.d;
            }
        }

    }
    return dist[t];
}

void Graph::test_correctness() {
    cout << "testing correctness..." << endl;
    for (int u = 1; u <= n; u++) {
        for (int v = 1; v <= n; v++) {
            for (int ts = t_min; ts <= t_max; ts++) {
                for (int te = t_min; te <= t_max; te++) {
                    int query = span_distance(u,v,ts,te);
                    int online = temporal_dijkstra(u,v,ts,te);
                    if (query != online) {
                        cout << "Incorrect query = " << query << ", online = " << online << endl;
                    }
                }
            }
        }
    }
}

void Graph::calculate_index_size() {
    int num_label = 0;
    for (int i = 0; i < out_label.size(); i++){
        for (int j = 0; j < out_label[i].size(); j++) {
            num_label++;
        }
    }

    for (int i = 0; i < in_label.size(); i++){
        for (int j = 0; j < in_label[i].size(); j++) {
            num_label++;
        }
    }
}